...
const SETTINGS_KEY = 'openkei.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('OpenKei');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      ...
      outputChannel?.appendLine(
        `[OpenKei] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    if (attempted) return;
    await context.globalState.update('openkei.sidebarAutoMoveAttempted', true);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.openkei');
      } catch (e) {
        outputChannel?.appendLine(`[OpenKei] workbench.view.extension.openkei failed: ${e}`);
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.focusChat', async () => {
      await vscode.commands.executeCommand('openkei.chatView.focus');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage('OpenKei: No active session');
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.restartApi', async () => {
      try {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage('OpenKei [Add to Context]:No active editor');
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage('OpenKei [Add to Context]: No text selected');
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (mentionPaths.length === 0) {
        vscode.window.showWarningMessage('OpenKei: No file selected to mention');
        return;
      }

      await vscode.commands.executeCommand('openkei.openSidebar');
      await new Promise((resolve) => setTimeout(resolve, 80));
      chatViewProvider?.addFileMentions(mentionPaths);

      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage('OpenKei: Some selected entries were skipped (folders or unsupported resources)');
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage('OpenKei [Explain]: No active editor');
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage('OpenKei [Improve Code]: No active editor');
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage('OpenKei [Improve Code]: No text selected');
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.newSession', () => {
      chatViewProvider?.createNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.showSettings', () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openkei.showOpenCodeStatus', async () => {
      ...
      const lines = [
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('openkei').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
